...
class gui:
    ...
    def page5(self, window4):
        ...
        window5.geometry("800x680+90+105")
        n_graph = Label(window5, text="Noise Graph Image")
        self.flag = 1
        n_graph.place(relx=.1, rely=.1, anchor="c")

        plt.figure(0)
        plt.hist(cv2.resize(self.cv_noise_img,(200,200))[self.y1_line_pt:self.y2_line_pt,self.x1_line_pt:self.x2_line_pt].ravel(), 256, [0, 256])
        ...
        Label(window5, image=self.noised).place(relx=.46, rely=.3, anchor="c")

        r_graph = Label(window5, text="Restored Graph Image")
        r_graph.place(relx=.1, rely=.5, anchor="c")
        plt.figure(1)
        plt.hist(cv2.resize(self.cv_image_out,(200,200))[self.y1_line_pt:self.y2_line_pt,self.x1_line_pt:self.x2_line_pt].ravel(), 256, [0, 256])    
        ...
        Label(window5, image=self.restored).place(relx=.46, rely=.7, anchor="c")
